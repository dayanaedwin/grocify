...
import { createOrder, deleteAllCartItems } from '../thunks';
...
export const OrderSummary: React.FC<IOrderSummary> = ({ orderInfo, currentStep, handleNext, setIsVisible }) => {
    ...
    const handleStepNavigation = async () => {
        if (currentStep === 3) {
            try {
                ...
                await dispatch(deleteAllCartItems());
                setIsVisible(true);
            } catch (error: any) {
                console.log(error);
            }
